...
export default function WorkspaceSettingsModal({ isOpen, onClose, workspace, onSave, userRole, workspaces = [], onDuplicateWorkspace, onDeleteWorkspace }) {
  ...
  const handleSave = () => {
    ...
    onSave(
      workspace.id, 
      { name: name.trim(), isPublic, allowCodeView, allowDomView },
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 backdrop-blur-md animate-fade-in p-4 text-slate-200">
      <div className="bg-slate-900 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] w-full max-w-md overflow-hidden relative border border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-slate-950 px-6 py-5 border-b border-slate-800 flex items-center justify-between shrink-0 relative z-10">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-white bg-slate-800 border border-slate-700 w-8 h-8 rounded-full flex items-center justify-center transition-colors cursor-pointer shrink-0">
        ...
        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1 relative z-10 bg-slate-900">
            ...
            <div className={`p-5 border rounded-2xl transition-all duration-300 ${!isPublic ? 'bg-slate-950 border-slate-800 shadow-inner' : 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)]'}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="pr-4">
                  ...
                  {/* ✨ Enhanced explicit status badge */}
                  <div className="mb-2">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isPublic ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-amber-500/10 text-amber-500 border border-amber-500/30'}`}>
                      Status: {isPublic ? 'Public & Shareable' : 'Strictly Private'}
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-400 leading-relaxed">
                    {isPublic 
                      ? "This project is currently visible on the Explore feed. Anyone with the share link can view it." 
                      : "This project is locked to your account. It will not appear on the Explore feed and share links are disabled."}
                  </div>
                </div>
                <button onClick={() => setIsPublic(!isPublic)} className={`w-11 h-6 shrink-0 flex items-center rounded-full p-1 transition-colors cursor-pointer ${isPublic ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-slate-700'}`}>
                ...
                <div className="pl-3 border-l-2 border-emerald-500/50 space-y-4 py-1">
                  <div className="flex items-center justify-between">
                    <div className="pr-4">
                      ...
                      <div className="text-[9px] text-slate-400">Allow users to inspect your HTML structure.</div>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="pr-4">
                      ...
                      <div className="text-[9px] text-slate-400">Allow users to view your compiled CSS.</div>
                    </div>
              ...
              {requireSwap && (
                <div className="mt-4 pt-4 border-t border-slate-800 animate-fade-in">
                  <div className="text-xs font-bold text-amber-400 mb-2 flex items-center gap-1.5"><i className="bi bi-exclamation-triangle-fill drop-shadow-[0_0_5px_rgba(251,191,36,0.8)]"></i> Private Limit Reached (Max {privateLimit})</div>
                  <p className="text-[10px] text-amber-500/80 mb-3 leading-relaxed">To make this project private, select an existing private project to become public. 🔄</p>
                  <select value={swapTargetId} onChange={(e) => setSwapTargetId(e.target.value)} className="w-full bg-slate-950 border border-amber-500/30 rounded-xl px-3 py-2.5 text-xs font-bold text-amber-400 focus:outline-none focus:border-amber-400 outline-none cursor-pointer">
                    {otherPrivateWorkspaces.map(ws => (<option key={ws.id} value={ws.id} className="bg-slate-900">{ws.name} (ID: {ws.id})</option>))}
                  </select>
                </div>
              )}
            </div>
          ...
          {/* ✨ NEW: Project Actions Section */}
          <div className="space-y-4 pt-2">
             <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Project Actions ⚡</label>
             <div className="grid grid-cols-2 gap-3">
                <button onClick={handleDuplicate} className="flex items-center justify-center gap-2 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 font-bold text-xs rounded-xl transition-colors cursor-pointer shadow-sm">
                   <i className="bi bi-files"></i> Duplicate
                </button>
                <button onClick={handleDelete} className="flex items-center justify-center gap-2 py-3 bg-red-500/10 hover:bg-red-600 border border-red-500/30 hover:border-red-500 text-red-400 hover:text-white font-bold text-xs rounded-xl transition-colors cursor-pointer shadow-sm">
        ...
        <div className="bg-slate-950 px-6 py-5 border-t border-slate-800 shrink-0 relative z-10">
          <button onClick={handleSave} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm rounded-xl transition-all active:scale-[0.98] cursor-pointer shadow-[0_0_15px_rgba(79,70,229,0.3)] flex items-center justify-center gap-2">
